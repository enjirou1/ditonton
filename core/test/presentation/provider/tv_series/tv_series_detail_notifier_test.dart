...
import '../../../../lib/domain/usecases/tv_series/get_tv_series_detail.dart';
import '../../../../lib/domain/usecases/tv_series/get_tv_series_recommendations.dart';
import '../../../../lib/domain/usecases/tv_series/get_watchlist_status.dart';
import '../../../../lib/domain/usecases/tv_series/remove_watchlist.dart';
import '../../../../lib/domain/usecases/tv_series/save_watchlist.dart';
import '../../../../lib/utils/state_enum.dart';
import '../../../../lib/presentation/provider/tv_series/tv_series_detail_notifier.dart';
...
void main() {
  ...
  group('Get TvSeries Recommendations', () {
    ...
    test('should update error message when request in successful', () async {
      // arrange
      when(mockGetTvSeriesDetail.execute(tId)).thenAnswer((_) async => Right(testTvSeriesDetail));
      when(mockGetTvSeriesRecommendations.execute(tId)).thenAnswer((_) async => const Left(ServerFailure('Failed')));
      // act
      await provider.fetchTvSeriesDetail(tId);
      // assert
  ...
  group('Watchlist', () {
    ...
    test('should execute save watchlist when function called', () async {
      ...
      when(mockGetWatchlistStatus.execute(testTvSeriesDetail.id)).thenAnswer((_) async => true);
      // act
    ...
    test('should execute remove watchlist when function called', () async {
      // arrange
      when(mockRemoveWatchlist.execute(testTvSeriesDetail)).thenAnswer((_) async => const Right('Removed'));
      when(mockGetWatchlistStatus.execute(testTvSeriesDetail.id)).thenAnswer((_) async => false);
      // act
    ...
    test('should update watchlist status when add watchlist success', () async {
      // arrange
      when(mockSaveWatchlist.execute(testTvSeriesDetail)).thenAnswer((_) async => const Right('Added to Watchlist'));
      when(mockGetWatchlistStatus.execute(testTvSeriesDetail.id)).thenAnswer((_) async => true);
      // act
      await provider.addWatchlist(testTvSeriesDetail);
      // assert
    ...
    test('should update watchlist message when add watchlist failed', () async {
      // arrange
      when(mockSaveWatchlist.execute(testTvSeriesDetail)).thenAnswer((_) async => const Left(DatabaseFailure('Failed')));
      when(mockGetWatchlistStatus.execute(testTvSeriesDetail.id)).thenAnswer((_) async => false);
      // act
      await provider.addWatchlist(testTvSeriesDetail);
      // assert
